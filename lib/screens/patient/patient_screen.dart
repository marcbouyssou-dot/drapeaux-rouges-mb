...
class _PatientScreenState extends State<PatientScreen> {
  ...
  Future<void> chargerPatients() async {
    final loadedPatients =
        await PatientSessionService.getPatients();

    final activePatient =
        await PatientSessionService.getPatientData();
  ...
  Future<void> creerDossierPatient() async {
    if (!consentementRgpd) {
      afficherMessage(
        'Le consentement RGPD doit être validé.',
      );
      return;
    }
    ...
    if (nomController.text.trim().isEmpty ||
      ...
      afficherMessage(
        'Merci de renseigner au minimum nom et prénom.',
      );
      return;
    }
    ...
    afficherMessage(
      'Patient créé et activé localement.',
    );
  }
        ...
        child: Column(
          children: [
            SwitchListTile(
              ...
              title: const Text(
                'Consentement patient recueilli',
              ),

              subtitle: const Text(
                'Obligatoire avant création du dossier local.',
              ),
            ),
  ...
  Widget _buildActivePatientCard() {
    if (codePatientActif == null) {
      return const _InfoCard(
        ...
        text:
            'Créez ou sélectionnez un patient avant une évaluation.',
      );
    }
  ...
  Widget _buildPatientsList() {
    if (patients.isEmpty) {
      return const _InfoCard(
        ...
        text:
            'Les patients créés localement apparaîtront ici.',
      );
    }
